"""在多个 PortraitHub 工作节点上执行运维操作的控制脚本。"""
...
def request_worker(base_url: str, args: argparse.Namespace) -> dict[str, Any]:
    ...
    with httpx.Client(timeout=timeout) as client:
        if action == "health":
            ...
            response = client.post(f"{base_url}/v1/admin/models/reload-config", headers=headers)
        elif action == "aliases":
            response = client.get(f"{base_url}/v1/admin/models/rollout/aliases", headers=headers)
        elif action == "warmup":
            ...
            response = client.post(f"{base_url}/v1/models/{model_id}/unload", headers=headers)
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="在多个 PortraitHub worker 上执行运维操作。")
    parser.add_argument(
        "--base-url",
        action="append",
        default=[],
