...
当前版本：0.18.5。本补丁版本修复视频任务与视频流解析结果在状态持久化后的帧预览恢复，并使控制台接口定义页在生产关闭公开 OpenAPI 文档时继续可用。公开 API、OpenAPI 基线和数据库 schema 保持兼容；真实模型、目标集群演练、生产容量观测及法律/产品审批仍是正式生产切换的前置条件。
...
- 视频任务详情按确定性的分析归档 ID 恢复持久化帧缩略图，外部 worker 或服务重启后不再只显示“无法预览”。
- 视频流详情读取最新解析完成事件的归档 ID，恢复实时解析结果图片。
- 归档没有可见图片时保留仍有效的内存预览，避免空归档覆盖已有结果。
- 调试台通过受 `access:read` 保护的控制台专用接口读取 OpenAPI；生产可继续关闭公开 `/openapi.json`。
- 接口定义页支持从已鉴权响应下载完整 `openapi.json`，浏览器验证 154 条路径、180 个接口操作正常显示。
- 本版本无公开 API、OpenAPI 基线或数据库 schema 变更，从 `0.18.4` 升级无需数据迁移。完整说明见 [0.18.5 发布说明](docs/releases/0.18.5.md) 和 [更新日志](更新日志.md)。
...
- 23 个页面中的 38 个原生表格和 4 个 Element Plus 表格统一使用全局表格规格，业务页面不直接使用 `ElPagination`。
- 服务质量 SLA 定义和模型注册影子结果补齐全局分页与连续序号。
- 图片分析归档和分析结果归档统一使用全局游标分页组件，删除页面私有加载更多实现和残留样式。
- 配置中心删除移动端独立卡片模板，桌面与移动端共用可横向滚动的标准表格和同一分页状态。
- 本版本无公开 API、OpenAPI 或数据库 schema 变更，从 `0.18.1` 升级无需数据迁移。完整说明见 [0.18.2 发布说明](docs/releases/0.18.2.md) 和 [更新日志](更新日志.md)。

## 0.18.1 平台性能、配置健壮性与交付优化

- 控制面实体投影改为条件 UPSERT 和 anti-join 清理，避免每次保存全量删除、重插实体及重建索引。
- PostgreSQL 快照在共享锁外预取，写入继续使用 revision CAS；阻塞持久化改走独立 `portrait-io` 线程池。
- 非法整数、浮点和布尔环境变量会告警并汇总；生产 profile 在启动阶段 fail-fast，且日志不记录原始配置值。
- GPU 发现异常增加告警；`.env.example` 增加 `BLOCKING_IO_THREAD_POOL_SIZE` 配置样例。
- GPU/CPU Docker 构建默认使用官方软件源，并允许通过 `APT_MIRROR`、`PIP_INDEX_URL` 适配国内、代理或隔离网络。
- 主 CI 拆为四个并行 job，缓存 Playwright Chromium；部署门禁持续检查并行结构和软件源参数化。
- 本版本无公开 API 或数据库 schema 变更，从 `0.18.0` 升级无需数据迁移。完整说明见 [0.18.1 发布说明](docs/releases/0.18.1.md)、[平台优化计划](docs/plans/PLATFORM_OPTIMIZATION_PLAN.md) 和 [更新日志](更新日志.md)。

## 0.18.0 商业产品能力、治理闭环与 Kubernetes 交付

- 商业控制面覆盖试用、激活、宽限、暂停、下线和关闭生命周期，支持定时变更、取消、并发版本检查、授权升级/降级/临时扩容/紧急授权、离线签名许可证和租户级并发控制。
- 计量体系保存不可变原始事件、幂等冲突与冲正记录，按租户、项目、应用、模型、能力、端点和资源归集图片、视频、GPU、存储、网络及第三方成本，并提供预算告警、配额耗尽预测和时区化日/月汇总。
- 模型调度与治理补齐动态批处理、租户公平、取消/超时/降级、来源与评估、审批状态、别名 CAS、可信热更新、预热、指纹校验、影子推理和回滚。
- 反馈样本池、标注交换、数据集血缘、不可变误差分析、只读阈值建议及候选发布决策形成数据闭环；视频任务支持可恢复上传、检查点、优先级队列、失败恢复和回调时间线。
- Webhook 增加签名投递记录、重试、死信和人工重投；Console Next 增加商业运营、服务质量、合规、模型注册和 Webhook 调试工作台，并完成桌面、平板、移动端及多浏览器验收。
- Kubernetes 发布物包含不可变稳定/灰度镜像、网络与安全策略、HPA/KEDA、PDB、GPU 亲和、迁移和发布前门禁；商业数据迁移、支持矩阵、OpenAPI 兼容、SDK 干净环境烟测和证据包工具同步交付。
- Python SDK 为稳定支持客户端，Node SDK 进入维护支持，Go/Java 继续作为实验性参考客户端；运行时与四套 SDK 版本统一为 `0.18.0`。
- 完整能力、升级步骤、兼容/回退边界、验证记录和未解除的生产阻塞项见 [0.18.0 发布说明](docs/releases/0.18.0.md)、[商业升级状态](docs/plans/COMMERCIAL_PRODUCT_UPGRADE_STATUS.md) 和 [更新日志](更新日志.md)。

## 0.17.0 强结构化契约、项目级隔离与生产能力

- `/v1` 成功与错误响应统一携带 `schema_version: "1.0"`、`request_id` 和稳定外层；核心解析接口补齐严格 OpenAPI 数据模型，向量字段按请求选择返回且保持可空字段稳定。
- 新增租户内项目目录和 `X-Project-ID` 上下文，人员库、任务、流、解析档案、复核、比对与推理按 `tenant_id + project_id` 隔离；API Key、JWT、OIDC、浏览器会话和 WebSocket 都执行项目授权。
- 接入应用、Webhook、调用日志及统计绑定项目；Console Next 与 Python、Node、Go、Java SDK 均支持项目选择，未指定时兼容使用 `default` 项目。
- PostgreSQL 新增访问状态 CAS、应用日配额原子计数和持久化调用日志/汇总；部署架构脚本和门禁同步覆盖新表与并发连接池。
- 生产模型模板扩展到人体检测、ReID、外观、人脸检测、人脸特征、姿态和步态七类能力；模板 SHA256 仍须替换为真实权重摘要后才能通过完整生产门禁。
- 完整迁移、兼容性、回退边界和验证记录见 [0.17.0 发布说明](docs/releases/0.17.0.md) 和 [更新日志](更新日志.md)。

## 0.16.0 控制台导航与数据表体验优化

- 左侧导航隐藏垂直滚动条，同时保留鼠标滚轮、触控板和键盘滚动能力；桌面端菜单项上下间隔由 2px 调整为 1px，较矮视口下“运维与合规”的选中边框可以完整显示。
- 控制台全部 19 个原生数据表和 4 个 Element Plus 数据表统一增加“序号”列，分页后序号连续；批量比对页的重复序号表头已修复。
- 配置中心“全部配置”表格启用单元格边框、分页和分类排序，默认每页 20 条，可切换 10/20/50 条；桌面表格与移动列表共用同一分页数据。
- 新增数据表序号契约测试，自动扫描表格实现，避免后续新增列表遗漏序号列。
- 本版本仅涉及控制台展示和交互优化，公开 PortraitHub v1 API 与后端数据契约保持兼容；升级无需数据迁移，部署时重新构建前端或镜像即可。
- 完整升级、回退和验证记录见 [0.16.0 发布说明](docs/releases/0.16.0.md) 和 [更新日志](更新日志.md)。
...
## 0.14.0 后端能力对齐与控制台功能补全

- 新增受管成员 CRUD、手机号/外部 subject 绑定、租户编辑与启停；OIDC 浏览器会话实时受成员和租户状态约束。
- 身份与权限页支持成员授权、租户管理和角色权限，并可为新租户创建带限流、配额和 scope 的默认应用。
- 模型中心补齐详情、预热、重载、配置重载、权重灰度、命中预览、回滚和发布审计；运维页补齐数据导出和增量备份。
- 图片分析新增人员轨迹；视频任务和实时流开放检测模型、ReID、阈值、采样、批次、超时和向量参数。
- 人员比对支持返回向量、批量异步和融合模态；检索支持阈值方案，人员注册支持元数据，特征重建支持模型和模态。
...
- 工作台取消“智能分析”中间层，直接提供总览、图片分析、视频任务、实时视频流、分析结果、人员比对、以图搜人和人员库；/analysis 不再保留。
- 登录页默认使用本地管理员账号，初始凭据为 admin / 123456；默认弱凭据仅允许本机使用，生产或远程访问必须更换密码和会话密钥。
- 支持 OIDC Authorization Code + PKCE 企业 SSO，外部角色/用户组映射为五个内置角色，浏览器会话统一执行租户隔离、RBAC、CSRF 和限流身份识别。
- 系统管理新增“身份与权限”，展示当前主体、认证来源、租户、角色和权限矩阵；企业用户生命周期仍由身份平台管理。
- API Key/JWT 保留为高级凭证入口；“开发者模式”仅改名为“调试信息”，原权限与脱敏行为不变。
- 完整升级说明、配置项和验证结果见《更新日志》的 0.13.0 条目。

## 0.12.1 配置与控制台体验修复

- `.env.example` 已与运行时设置和 Docker Compose 使用的配置项对齐，补齐图片像素/并发/检测参数、视频任务重试、接入状态文件、加密 KDF、限流来源、向量后端和 worker 设备等示例；本地开发状态文件使用 `runtime-state` 下的安全默认路径。
- Console Next 不再缓存服务端 `index.html`，前端构建后会立即读取新的哈希资源，避免旧 HTML 引用已删除的 `index-*.js` 造成 404。
- 路由切换默认回到页面顶部；顶部栏显示页面标题和右侧说明，正文不再重复显示标题；说明文字恢复为真实中文，空租户时隐藏空标签。
- Console Next 全部产品页面的普通业务文案与接口枚举统一中文显示；仅保留 SLO、GPU、JWT、HTTP、IoU、SDK、编程语言、后端产品名、模型 ID、接口路径和错误码等必要技术专名。
- 版本统一为 `0.12.1`，Python、Node、Go、Java SDK 的 User-Agent/版本常量同步更新。
