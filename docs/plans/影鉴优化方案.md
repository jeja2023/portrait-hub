...
> [!UPDATE]
> 2026-07-13 / v0.6.0 已完成三阶段平台收口：按 `PORTRAIT_HUB_THREE_PHASE_OPTIMIZED_PLAN.md` 补齐接入中心、应用 API Key/JWT、SDK 批量与视频示例、OpenAPI、API Playground、服务端调用日志、错误码目录、Webhook、SLO 面板、rollout 审计读回、轨迹审阅标注池、评估中心只读阈值推荐、审计链校验、审计事件筛选和备份快照读回；平台 strict readiness 为 `ok=true`、`strict_failure_count=0`、`checks=180`，完整 strict 仍按预期保留 5 个真实模型能力接入项。
