> 2026-07-14 / v0.7.1 已完成租户目录与管理员一键开通体验收口：单租户 API Key/JWT 可自动解析租户，管理员可通过 `/v1/access/tenants` 或控制台“接入中心”只填写租户名称生成 `tenant_id`、默认接入应用和一次性 API Key；业务应用默认不具备 `tenants:read` / `tenants:write`，旧版 `X-Tenant-ID + 凭证` 继续兼容，多租户 JWT 仍需显式选择租户。
