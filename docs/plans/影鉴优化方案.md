> 2026-07-14 / v0.8.0 已完成本轮 6 项平台升级：持久化视频队列与独立 worker、全局 API_TOKEN 租户边界、分块上传、批量调用统计、严格类型/生产门禁和结果灯箱焦点管理；按约定暂不包含移动端导航优化。上一轮 v0.7.1 的租户目录与管理员一键开通体验继续兼容。

> 2026-07-18 / v0.11.1 更新：旧 frontend/console 已删除，Console Next 是唯一生产控制台；本文后续提到 frontend/console 的段落均为历史优化记录，不再作为当前开发入口。
# PortraitHub 项目优化与扩展方案

本报告基于对 PortraitHub v0.5.24 全部源码、配置、测试、部署模板和更新日志的深入审查，从架构安全、性能瓶颈、代码质量、可运维性和功能扩展五个维度提出优化建议。

> [!UPDATE]
> 2026-07-13 / v0.6.2 已完成控制台信息架构二次收口：侧边栏重组为「总览 / 智能分析 / 比对检索 / 人员库 / 接入中心 / 模型与评估 / 运维合规」，新增 `frontend/console/views/navigation.js` 作为导航配置模块，`app.js` 改为消费 `PortraitConsoleModules.navigation`；人员库新增独立“特征重建”页复用 `/v1/gallery/reindex`，并将“刷新全部”收敛为按当前视图最小刷新。`npm run check` 与 `git diff --check` 已通过。

> [!UPDATE]
> 2026-07-13 / v0.6.0 已完成三阶段平台收口：按 `PORTRAIT_HUB_THREE_PHASE_OPTIMIZED_PLAN.md` 补齐接入中心、应用 API Key/JWT、SDK 批量与视频示例、OpenAPI、接口调试台、服务端调用日志、错误码目录、Webhook、SLO 面板、rollout 审计读回、轨迹审阅标注池、评估中心只读阈值推荐、审计链校验、审计事件筛选和备份快照读回；平台 strict readiness 为 `ok=true`、`strict_failure_count=0`、`checks=180`，完整 strict 仍按预期保留 5 个真实模型能力接入项。
...
> [!UPDATE]
> 2026-06-15 / v0.5.29 已完成剩余计划的工程收口：`core.py` 显式导出与路由通配导入清理、数据库池化、本地图库 WAL、JWT 非对称验签、Qdrant 客户端复用、关键全局状态锁保护、视频抽帧 async 隔离、追踪 Union-Find/大批量关联优化、Prometheus 缓存、结构化日志上下文、增强健康检查、Docker 多阶段构建、配置热重载、WebSocket 推送、异步批量比对/检索、模型灰度、OpenTelemetry、增量导出/备份、独立控制台、依赖/版本治理、聚焦 `mypy --strict` 类型门禁、迁移/备份/压测工具和测试/验收门禁均已实现并验证。本文保留原始建议正文作为设计记录，不逐项改写历史标记。

> [!UPDATE]
> 2026-06-21 / v0.5.36 已完成新一轮安全、运行时和数据路径审计收口：限流身份从可伪造租户头切换为客户端 IP/认证身份，`/ready` 匿名响应不再泄露依赖明细，模型 LRU 避免驱逐在途推理 bundle，内存 gallery 检索改为锁内快照，PostgreSQL 快照读取与图库替换降低全表中间态和逐行写入成本，Redis/S3 热路径客户端复用，批量比对补齐上限，rollout/model detail 错误边界脱敏，兼容门面改为显式导入并清理 ruff 问题。详细状态和后续生产验证项见 `SECURITY_RUNTIME_AUDIT_FIX_PLAN.md`。

> [!UPDATE]
> 2026-06-21 / v0.5.37 已完成 strict mypy backlog 的分块收口：全仓 `tools/type_check.py` 现在可在本机稳定执行 strict mypy，cache 固定到 `.codex-tmp/mypy-cache`；PostgreSQL、gallery、threshold、model config、metrics、runtime、inference、route、server 和工具脚本的 `Any`、Optional、JSON、ndarray 与重导出边界已按模块收紧。已通过 `tools/type_check.py` 的 141 个源码文件检查，以及核心契约、算法、数据后端和生产就绪测试组合。

> [!UPDATE]
> 2026-06-23 / v0.5.39 已完成影鉴业务控制台的信息架构与控件密度优化：侧边栏由 9 个并列按钮重构为「总览 + 智能解析/人员库/视频分析/运维治理」四个可展开分组（原生 `<details>`），`setView` 自动展开活动分组并兼容旧视图名回退；「人员库」拆分为 注册/搜人/管理 三子页、「治理」拆分为 阈值/数据 两子页，结果渲染分流到各子页独立容器；表单控件按内容收窄（数字框 160px、短文本/下拉 300px、长字段占满），结果区与 JSON 区高度下调。改动仅限 `frontend/console/*` 与一处过期标题断言，`tests/test_api_contract.py` + `tests/test_security_auth.py` 共 73 项通过。

> [!UPDATE]
> 2026-07-11 / v0.5.58 已完成批量并发稳定化维护：多图片解码、图库批量检索和批量人像/衣着比对统一改为有界并发，失败时取消进行中工作并停止启动后续项；进度回调串行化，图片像素上限改为完整加载前显式拒绝；新增并发边界回归测试，并在 `.venv` 下通过 strict mypy。

> 2026-07-11 / v0.5.55 已完成后审计维护收口：修复模块拆分后图库 enroll、视频任务取消和 retention cleanup 的 route-level hook 漂移，恢复对象存储替换、audit 失败回滚、特征持久化失败清理和保留清理补偿测试；strict mypy 与完整 pytest 重新全绿；批量异步图片读取统一接入 `read_limited_upload()`，避免超限图片完整读入内存；生产就绪静态规则同步到新的补偿实现。完整 strict 生产就绪仍保留 5 个真实模型资产接入项，作为后续模型交付工作继续推进。
> 2026-07-03 / v0.5.54 已完成后审计优化升级：修复 CI/部署门禁阻断，新增源码 UTF-8 BOM 卫生检查，控制台配置拆分为 `console.config.js` 并纳入 API/部署/生产就绪门禁，图库 mutation 回滚和对象清理逻辑抽取为独立服务模块，新增 `package.json` 标准化 `npm run check`，并以 `POST_AUDIT_UPGRADE_PLAN.md` 固化下一阶段真实数据栈、模型灰度、控制台模块化、备份恢复、运维安全和容量评估扩展矩阵。
