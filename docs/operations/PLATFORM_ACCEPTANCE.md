...
本文档定义 0.18.5 视频结果预览与控制台接口定义修复补丁及其继承的既有平台和商业产品能力验收范围，并明确区分自动化完成、目标环境观察和人工审批。
...
- 视频任务和视频流状态继续剥离嵌入式图片正文，控制台通过分析归档恢复缩略图；无可见归档图片时保留内存预览。
- 生产关闭 `/docs`、`/redoc` 和 `/openapi.json` 时，受 `access:read` 保护且不进入公开契约的控制台定义接口仍可用。
- Console Next Vitest 48 项、ESLint、Vue TypeScript 和 production build 通过；定向 Python 契约、Ruff、版本一致性和支持矩阵检查通过。
- 浏览器确认视频任务、视频流帧图片和接口定义页正常显示，接口定义为 154 条路径、180 个操作，控制台无错误日志。
- 公开 `/v1` API、OpenAPI 基线、数据库 schema 和生产阻塞状态未改变；生产发布决策继续保持阻塞。
