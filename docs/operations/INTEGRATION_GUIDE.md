...
- 离线视频上传支持浏览器 `video/*` MIME 类型以及 `.mp4`、`.mov`、`.m4v`、`.avi`、`.mkv`、`.webm` 扩展名。反向代理和应用层都应允许完整请求体；生产环境的 `MAX_REQUEST_BODY_BYTES` 必须位于 `MAX_VIDEO_BYTES + 1 MiB` 到 `MAX_VIDEO_BYTES + 16 MiB` 之间，以容纳 multipart 边界和字段。
