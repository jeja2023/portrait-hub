...
> 2026-07-19 / 0.13.0：工作台取消“智能分析”中间菜单并删除 /analysis 入口；登录升级为本地管理员、OIDC 企业 SSO 与高级 API Key/JWT 三类入口；浏览器会话统一接入 HttpOnly Cookie、CSRF、租户隔离和 RBAC；系统管理新增“身份与权限”，原“开发者模式”仅改名为“调试信息”且权限保持不变。
>
> 2026-07-19 / 0.12.1 补丁：服务端不缓存 Console Next `index.html`，前端重建后不会继续引用旧哈希资源；路由切换恢复滚动顶部；顶栏采用标题与右侧说明的单行结构，正文隐藏重复标题，空租户不显示空标签。
> 2026-07-19 / 0.12.0 补充：SLO 改用带保留完整性标记的服务端聚合；WS degraded 时使用受控详情轮询；会话按 expires_at 失效；页签、筛选和详情深链统一由路由 query 驱动；多 worker/多副本通过 REDIS_URL 使用 Redis TTL 与 Lua 原子单次消费 ws-ticket。

> 2026-07-18 / 0.11.2 补充：二次复核后，WS 主凭证禁止 query 回退，`/v1/*` 响应统一 no-store，默认 CSP 去除 `unsafe-inline` 与 jsdelivr；导航以路由 meta 为唯一来源，流详情使用 `/analysis/stream/:streamId` 深链，错误与动态状态满足可访问通告并携带 request_id。
...
- 本地账号与 OIDC 使用签名 HttpOnly、SameSite Cookie；写请求使用独立 CSRF Cookie 与 X-CSRF-Token，用户名和密码不进入浏览器存储或 URL。
- API Key 与 JWT 只存 sessionStorage，并收进高级凭证登录；显式非法凭证不得回退到已有浏览器会话。租户优先由凭证或浏览器会话自动解析。
- 默认本地管理员 admin / 123456 仅允许 loopback；生产或远程使用默认密码/默认会话密钥时自动禁用。企业用户生命周期由 OIDC 身份平台管理。
- WebSocket 浏览器连接只使用短期、单次、租户与资源绑定 ticket。配置 `REDIS_URL` 时使用 Redis TTL 与 Lua 原子 `GET + DEL`，支持跨 worker/副本共享；无 Redis 的进程内有界存储仅用于本地开发和测试。

### 运维就绪契约

- 调试台必须覆盖批量检索、批量比对和视频流注册/查询，并展示端点模板、HTTP 状态、错误码与 request_id。
- SLO 面板优先使用租户调用日志聚合，必须展示时间覆盖与保留完整性；缺少 access:read 或窗口无数据时明确回退 Prometheus，不得把达到保留上限的窗口标记为完整 30 天。成功率来源、错误预算、推理/排队分位数和设备队列必须进入脱敏原始数据。
...
- 默认业务视图不直接显示 JSON；调试信息中的原始数据先脱敏。
...
2026-07-19 / 0.12.0 发布复验结果：

- `python tools/type_check.py`：通过，177 source files。
- `python -m ruff check app tools tests`：通过。
- `python -m pytest -q`：通过，549 passed / 4 skipped。
- `go test ./...`：通过；Java SDK 主源码 `javac` 编译通过。
- `npm run check`：通过，Vitest 5 files / 13 tests，Vite production build 通过。
- `npm run console:e2e`：通过，15 passed / 0 skipped。
- `deploy_check --json --import-app`：ok=true。
- `portrait_production_readiness --scope platform --strict`：ok=true，strict_failure_count=0。
