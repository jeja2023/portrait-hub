...
本文档面向当前 PortraitHub 项目，说明如何在 Ubuntu 服务器上通过 Docker Compose 部署 GPU 推理服务。文档按仓库版本 0.18.5、当前 Dockerfile、docker-compose.yml、.env.example、商业数据迁移和生产门禁实现编写。
...
### 17.1 从 0.18.4 升级到 0.18.5

0.18.5 是视频结果预览与控制台接口定义修复补丁，公开 API、OpenAPI 基线、数据库 schema 和持久化格式不变，不需要执行数据迁移。必须成套重建 API、Console、视频任务 worker 和视频流 worker，使状态落盘与归档预览恢复逻辑保持一致。
...
升级后验证视频任务详情、视频流详情和调试台接口定义页。生产可保持 `ENABLE_API_DOCS=false`：公开 `/openapi.json` 应为 404，具备 `access:read` 的控制台仍应加载 154 条路径、180 个接口操作。0.18.5 没有数据迁移，可以回退到完整归档的 0.18.4 API、worker 和前端镜像；完整边界见 `docs/releases/0.18.5.md`。

### 17.2 从 0.18.3 升级到 0.18.4
...
默认构建使用 Ubuntu/PyPI 官方源。国内、代理或隔离网络可按需传入 `APT_MIRROR` 和 `PIP_INDEX_URL`，不要修改并提交 Dockerfile：
...
升级后验证两个 API worker 的 `/health`、`/ready`、控制面读写、GPU 清单、状态持久化和审计写入，再执行平台范围严格门禁。`BLOCKING_IO_THREAD_POOL_SIZE` 可结合单进程并发、PostgreSQL 连接池和磁盘延迟调整；未显式设置时使用运行时默认值。

0.18.1 没有数据格式迁移，可以回退到已归档的 0.18.0 镜像。回退后仍应保留已经修正的环境配置；不要依赖 0.18.0 对非法类型值的静默回退。完整边界见 `docs/releases/0.18.1.md`。

### 17.6 从 0.17.0 升级到 0.18.0
...
docker save -o portrait-hub-0.18.5-images.tar \
  portrait-hub-gpu-worker-0 \
  portrait-hub-gpu-worker-1 \
  portrait-hub-portrait-video-job-worker \
  portrait-hub-portrait-stream-worker
...
- [ ] 0.18.5 视频任务/视频流归档预览恢复、公开文档关闭时的控制台接口定义和完整定义下载已验证。
