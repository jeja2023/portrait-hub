...
const shadowPager = useTablePagination(shadowResults);
  ...
  <div>
    ...
    <ElSkeleton :loading="loading" :rows="6" animated>
      ...
      <section class="tool-surface"><ElTabs v-model="tab" class="page-tabs">
        ...
        <ElTabPane :label="`影子结果 (${shadowResults.length})`" name="shadow"><EmptyState v-if="!shadowResults.length" title="尚无影子对比结果" description="影子发布采样后会记录主模型与候选模型的一致性和延迟差异。" /><template v-else><div class="table-wrap"><table class="data-table"><thead><tr><th class="sequence-column">序号</th><th>主模型</th><th>影子模型</th><th>采样</th><th>一致</th><th>延迟差异</th><th>时间</th></tr></thead><tbody><tr v-for="(item,index) in shadowPager.items" :key="index"><td class="sequence-column">{{ shadowPager.startIndex + index + 1 }}</td><td><code>{{ item.source_model_id }}</code></td><td><code>{{ item.shadow_model_id }}</code></td><td>{{ item.sampled ? '是' : '否' }}</td><td>{{ item.agreement == null ? '--' : item.agreement ? '一致' : '不一致' }}</td><td>{{ item.latency_delta_ms ?? '--' }} ms</td><td>{{ formatTimestamp(item.created_at) }}</td></tr></tbody></table></div><DataTablePagination v-model:page="shadowPager.page" v-model:page-size="shadowPager.pageSize" :total="shadowPager.total" /></template></ElTabPane>
      </ElTabs></section>
    </ElSkeleton>
